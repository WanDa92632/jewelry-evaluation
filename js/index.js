let chosenMap = {};
let s =
    layui.use(['form', 'carousel', 'layer', 'element','tree'], function () {
        var form = layui.form,
            $ = layui.jquery,
        ...
        form.on('select(parameter)', function (data) {
            ...
            if (value === "") {
                ...
                $.get("/jewelryEvaluation/frontend/jewelry/parametervalue?parameterValueCode=" + value, function (result, status) {
                    if (isSuccess(result)) {
                        let parameterValue = result.data.parameterValue;
                        if (parameterValue.parameterIntroduction !== null) {
                            let parameterIntroduction = parameterValue.parameterIntroduction;
                            if (parameterIntroduction !== null && parameterIntroduction.length > 0) {
                                let str = "<p>";
                                str += parameterIntroduction;
                                // 预览图片按钮
                                if (parameterValue.imgUrl !== null||parameterValue.imgUrl !== '') {
                                    str += "<a href='javascript:' pid='" + parameterValue.id + "' class='layui-btn layui-btn-xs layui-btn-radius layui-btn-primary' id='view_image'>点击查看预览图片</a>";
                                }
                                str += "</p>";
                                $(id + "Content").html(str);
        ...
        $('#parameter').on('click', '#view_image', function () {
            ...
            $.getJSON('/jewelryEvaluation/frontend/jewelry/parameterImg', {id: pid}, function (result) {
                let parameterImg = result.data.parameterImg;
                console.log(parameterImg)
                layer.photos({
                    photos: parameterImg
